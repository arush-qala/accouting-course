...
const FastTransferContext = (
  <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100 text-sm text-indigo-900 mb-4">
    <h4 className="font-bold mb-2">FastTransfer Ltd — Customer Analysis</h4>
    <div className="overflow-x-auto">
        <table className="w-full text-left text-xs">
            <thead>
                <tr className="border-b border-indigo-200">
                    <th className="pb-1">Customer</th><th>Revenue</th><th>Prod. Cost</th><th>Calls</th><th>Reports</th>
                </tr>
            </thead>
            <tbody>
                {rows.map((row, i) => (
                    <tr key={i}>
                        <td>{row.c}</td><td>{row.r}</td><td>{row.pc}</td><td>{row.calls}</td><td>{row.rep}</td>
                    </tr>
                ))}
            </tbody>
        </table>
    ...
    <div className="mt-2 text-xs">
        <strong>Rates:</strong> Service Call: £200 | Custom Report: £500
    </div>
  </div>
);
